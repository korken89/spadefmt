...
| `multiline_string` | A string literal holding a raw newline is written like code lines: trailing whitespace inside the string is stripped (this pair: the `else` message `"line one   ` loses its three spaces), and at any nesting depth greater than zero the writer re-indents the continuation line, four more spaces on every pass (`let s = "a` / `b";` in a unit body prints `b";` at column 4, then 8), a facet no golden can pin because goldens must be fixed points. | `spadefmt` | The string's bytes are untouched: the spaces stay and `line two"` keeps its column. |
